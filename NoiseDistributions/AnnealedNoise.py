...
class AnnealedNoiseDistr(BaseNoiseDistr):
    ...
    def calculate_noise_distr_reward(self, noise_distr_step, entropy_reward, T):
        return -(noise_distr_step - T*entropy_reward)
    ...
    def calc_noise_step(self, jraph_graph, X_prev, X_next, model_step_idx, node_gr_idx, T):
        ...
        Noise_Energy_per_graph = jnp.squeeze(Noise_Energy_per_graph, axis = -1)
        return (-1)*gamma_t*Noise_Energy_per_graph

    @partial(jax.jit, static_argnums=(0,))
    def calc_noise_reward(self, jraph_graph, spin_logits_prev, spin_logits_next, X_prev, gamma_t, node_gr_idx):
        Noise_Energy_per_graph, _, _ = self.vmapped_relaxed_energy_for_Loss(jraph_graph, spin_logits_prev, node_gr_idx)
        Noise_Energy_per_graph = jnp.squeeze(Noise_Energy_per_graph, axis = -1)
        return (-1)*gamma_t*Noise_Energy_per_graph
