...
class CombinedNoiseDistr(BaseNoiseDistr):
    ...
    def calculate_noise_distr_reward(self, noise_distr_step, entropy_step, T):
        ...
        return T*  entropy_step - noise_distr_step
    ...
    @partial(jax.jit, static_argnums=(0,))
    def calc_noise_step(self, jraph_graph, X_prev, X_next, model_step_idx, node_gr_idx, T):
        ### TODO exchange beta_t with time_step?
        noise_step_cat = self.CategoricalNoseDist.calc_noise_step(jraph_graph, X_prev, X_next, model_step_idx, node_gr_idx, T)
        noise_step_anneal = self.AnnealedNoiseDist.calc_noise_step(jraph_graph, X_prev, X_next, model_step_idx, node_gr_idx, T)
        overall_noise_step = noise_step_cat + noise_step_anneal
        #noise_distr_step_dict = {"cat": noise_step_cat, "anneal": noise_step_anneal, "overall": res}
        return overall_noise_step
