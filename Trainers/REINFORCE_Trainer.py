...
class Reinforce(Base):
    ...
    def get_loss(self, params, graphs, energy_graph_batch, T, key):
        return self.diffusion_loss(params, graphs, energy_graph_batch, T, key)
    ...
    @partial(jax.jit, static_argnums=(0,))
    def diffusion_loss(self, params, graphs, energy_graph_batch, T, key):
        print("function is being jitted")
        overall_diffusion_steps = self.n_diffusion_steps * self.eval_step_factor
        X_prev, one_hot_state, log_p_uniform, key = self.model.sample_prior(energy_graph_batch, self.N_basis_states,
                                                                            key)
        ...
        for i in range(overall_diffusion_steps):
            model_step_idx = int(i/self.eval_step_factor)
            key, subkey = jax.random.split(key)
            batched_key = jax.random.split(subkey, num=self.N_basis_states)


            out_dict, _ = self.vmapped_make_one_step(params, graphs, X_prev, model_step_idx, batched_key)
