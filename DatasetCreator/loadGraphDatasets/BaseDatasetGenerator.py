	...
	def solve_graph(self, H_graph, g, thread_fraction) -> (float, float, list, float, jraph.GraphsTuple):
		...
		if self.gurobi_solve:
			if self.problem == "MaxCut":
				...
				_, Energy, solution, runtime = GurobiSolver.solveMIS_as_MIP(H_graph_compl, time_limit=self.time_limit)
				...
				H_graph_compl = jutils.from_igraph_to_jgraph(g, double_edges=False)
				_, Energy, solution, runtime = GurobiSolver.solveMVC_as_MIP(H_graph, time_limit=self.time_limit)
				return Energy, None, solution, runtime, H_graph_compl
